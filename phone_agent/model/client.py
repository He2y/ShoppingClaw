...
class ModelClient:
    ...
    def request(self, messages: list[dict[str, Any]]) -> ModelResponse:
        ...
        action_markers = ["finish(message=", "do(action=", "<tool_call>", "<answer>"]
    ...
    def _parse_response(self, content: str) -> tuple[str, str]:
        ...
        Parsing rules:
        1. If content contains '<tool_call>', everything before is thinking,
           the <tool_call>...</tool_call> block is action (Qwen-VL format).
        ...
        # Rule 1: Check for <tool_call>...</tool_call> (Qwen-VL format)
        tool_call_match = re.search(r'<tool_call>\s*(.*?)\s*</tool_call>', content, re.DOTALL)
        if tool_call_match:
            thinking_part = content[:tool_call_match.start()].strip()
            # Remove "Thought:" and "Action:" prefixes from thinking
            thinking_lines = []
            for line in thinking_part.split('\n'):
                line_stripped = line.strip()
                if line_stripped.startswith('Thought:'):
                    thinking_lines.append(line_stripped[8:].strip())
                elif line_stripped.startswith('Action:'):
                    pass  # Skip Action line, it's a brief description
                elif line_stripped:
                    thinking_lines.append(line_stripped)
            thinking = ' '.join(thinking_lines)
            action = tool_call_match.group(0)  # Include <tool_call> tags
            return thinking, action

        # Rule 2: Check for <answer>...</answer> XML tag format (GLM-4V / AutoGLM)
        # Must be checked BEFORE do()/finish() because <answer> may wrap them
        ...
        # Rule 3: Check for finish(message=

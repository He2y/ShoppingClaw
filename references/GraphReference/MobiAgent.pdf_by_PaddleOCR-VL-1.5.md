...
<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//bee42630-38fa-4b7f-9eca-e7364d0feeea/markdown_2/imgs/img_in_image_box_253_166_971_458.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A31Z%2F-1%2F%2Fb4cde791e805e09c13e60bfa01f4d931ec274a3a6f24ac75a63b2a0553ca700e" alt="Image" width="58%" /></div>


<div style="text-align: center;"><div style="text-align: center;">Figure 1: Data Collection Pipeline with Agent Self-evolving</div> </div>


crucial for training intelligent and robust agent models. To address this issue, we propose a VLM-based reasoning reconstruction approach which leverages Gemini-2.5 to reconstruct the agent's reasoning process based on original trajectories.
...
• Data Redistribution: Different trajectories tend to share similar prefixes while having diverse suffixes. When constructing training data, we perform sampling on the prefix actions while keeping all the suffix actions in the dataset, ensuring a more balanced distribution.
...
• Prompt Generalization: In real-world scenarios, agent’s input task descriptions varies across different users. However, each trajectory obtained during the initial data collection has only one corresponding task description, which can lead to weaker generalization abilities of the agent. Therefore, we assign more semantically equivalent task descriptions to every trajectory to build a more comprehensive training dataset.

• Corner-case Enhancement: With the single-step dataset obtained in Section 2.1, we enhance the agent's ability to handle more complicated corner cases (e.g., closing pop-up ads, terminating current task or returning to the previous page upon execution errors) by mixing single step desicion with history-based decision at training time. Experiments show that at test time, decider has stable capability of handling these problems at any time step without being interfered by the action history.
...
{"reasoning": "Your reasoning here", "action": "The next action (one of click, input, swipe, wait, done)", "parameters": {"param": "value"}}

Grounder:
<image>
Based on the screenshot, user's intent and the description of the target UI element, provide the element's bounding box using absolute coordinates.
...
Grounding GRPO. Firstly, we train the grounder with a rule-based reward function using the UI grounding dataset in Section 2. The reward function consists of two parts: an IoU reward  $ R_{iou} $ and a center-point reward  $ R_{center} $, reflecting the precision of the bounding box prediction of the grounder:

 $$ \begin{aligned}R_{iou}=\begin{cases}\alpha,&IoU\left(b_{pred},b_{gt}\right)>\beta,\\0,&otherwise.\end{cases},R_{center}=\begin{cases}1-\alpha,&Center\left(b_{pred}\right)is inside b_{gt},\\0,&otherwise.\end{cases},\\R&=R_{iou}+R_{center},\end{aligned} $$ 

where  $ b_{\text{pred}}, b_{gt} $ are the predicted and ground-truth bounding boxes,  $ \text{IoU}(\cdot) $ is the Intersection over Union function, and  $ \alpha, \beta $ are hyperparameters.
...
Specifically, for each response generated in the rollout stage, if the action is a non-click operation, we apply a binary reward  $ R_{content} $ by exact matching:

 $$ R=R_{content}=\begin{cases}1,&a_{pred}=a_{gt},\\0,&otherwise.\end{cases}, $$ 

where  $ a_{\text{pred}} $,  $ a_{\text{gt}} $ are the predicted and ground-truth actions. For click action, the grounder predicts a bounding box of the target element in decider's response, and directly returns  $ R = R_{\text{center}} $ as the final reward.
...
<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//5a64cc61-6006-4a05-be8b-04e1f673fd5d/markdown_0/imgs/img_in_image_box_217_144_1007_769.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A50Z%2F-1%2F%2Fd63e3d9a4787aeb1a3225ad60addc21971edf40b076f5e8cc8b1fd420218c556" alt="Image" width="64%" /></div>


<div style="text-align: center;"><div style="text-align: center;">Figure 2: Multi-Agent Architecture using the AgentRR Framework</div> </div>


latent memory, enabling swift execution of familiar actions while preserving the flexibility required for new situations.

### 4.2 Prefix Reusability of GUI tasks

We observe the prefix reusability of GUI tasks, which relies on the following insights:
...
ActTree is a concrete implementation mechanism that accelerates mobile tasks by leveraging the principles of AgentRR. As shown in Figure 3 ActTree maintains a tree-structured representation analogous to UI Transition Graphs. Each node  $ v \in \mathcal{V} $ in the tree corresponds to a unique UI state, denoted as  $ \mathcal{UI}_u $, and maintains a set of outgoing edges  $ \mathcal{E}_u = \{e_1, ..., e_n\} $. Each edge  $ e = (u, v) \in \mathcal{E} $ represents a state transition, which is characterized by: (1) an action  $ \mathcal{A}_e $ that triggers this transition;

<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//5a64cc61-6006-4a05-be8b-04e1f673fd5d/markdown_1/imgs/img_in_image_box_223_140_925_573.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A51Z%2F-1%2F%2F9af1877c12b49d2cb81fed22b7a7f443e43d7feddc1b904c009d30bf53a03e6a" alt="Image" width="57%" /></div>


<div style="text-align: center;"><div style="text-align: center;">Figure 3: Construction of the ActTree Structure During Mobile Task Execution</div> </div>
...
The task embedding model is responsible for computing the similarity between two tasks, which is measured as the cosine similarity between their respective embedding vectors. Below, we provide the formal representation of the embedding at a specific layer of the ActTree for a given task.
 ...
 $$ v_{i}^{(l)}=f(T_{i},l)\in\mathbb{R}^{d}. $$ 

Where:

 $ T_{i} $ represents the i-th task.
...
•  $ v_{i}^{(l)} $ is the embedding vector corresponding to the task  $ T_{i} $ and the layer l.

- d is the dimension of the embedding vector, and  $ R^{d} $ represents the d-dimensional real-valued vector space where the embedding vector resides.
...
To determine whether the action prefix of task $T_i$ can be reused by task $T_j$, we compare the cosine similarity of task embedding $S(v_i^{(l)}, v_j^{(l)})$ with a predefined similarity threshold $\tau_1$. The stage-1 decision of whether $T_i$ can reuse the actions of $T_j$ at the $l$-th layer in ActTree is formally expressed as:
 ...
 $$ \mathrm{Embed-Reuse}^{(l)}(T_{i},T_{j})=\begin{cases}\mathrm{True},&if S(v_{i}^{(l)},v_{j}^{(l)})\geq\tau_{1}\\ \mathrm{False},&otherwise\end{cases}. $$ 
...
Another approach to implementing the latent model is to leverage task reranking. Given two candidate tasks, the latent model determines whether the action at a specific layer of ActTree can be reused, and directly return a score (the logit of "yes" token) between 0 and 1. The task reranking mechanism serves as a complementary method to task embedding. In our implementation, we first utilize the task embedding model to compute similarity scores between the current task and all historical tasks for the current layer, then select a set of candidate tasks C with the highest similarity scores:
 ...
 $$ \mathcal{C}=\{T\in\mathcal{T}_{\mathrm{h i s t}}\mid\mathbf{E m b e d-R e u s e}^{(l)}(T,T_{c})\}, $$ 

where  $ T_{c} $ is the current task,  $ \mathcal{T}_{\text{hist}} $ represents all historical tasks recorded in outgoing edges of the current ActTree node.

Subsequently, we perform task reranking by comparing each candidate task with the current task. If there exists at least one candidate whose reranking score with the current task is greater than another predefined confidence threshold  $ \tau_{2} $, the action at the corresponding node in ActTree is considered reusable for the current task. This process enables the latent model to make more accurate reuse decisions compared to relying solely on the embedding model. The stage-2 (final) reuse decision is expressed as:

 $$ \mathrm{Rerank-Reuse}^{(l)}=\begin{cases}\mathrm{True},&if\exists T_{i}\in\mathcal{C}such that g(T_{i},T_{c},l)\geq\tau_{2}\\ \mathrm{False},&otherwise\end{cases}. $$ 
...
For the task embedding model, for a given ActTree layer l and a task T, other tasks that share an l-step action prefix with T are considered positive samples, while tasks sharing fewer than l steps are considered negative samples. Formally, for  $ v = f(T_i, l) $, the positive and negative samples are sampled in the following way:
 ...
 $$ v^{+}=f(T_{s},l),\operatorname{pre}(T_{s},T_{i})\geq l, $$ 

 $$ v^{-}=\left\{f(T_{t},l)\mid\operatorname{pre}(T_{t},T_{i})<l\right\}, $$ 

where  $ \mathrm{pre}(\cdot,\cdot) $ gives the length of the shared action prefix of two tasks, and we calculate the InfoNCE [24] loss with a temperature parameter  $ \tau $:

 $$ \mathcal{L}=\mathbb{E}_{v}\left[-\ln\frac{e^{S(v,v^{+})/\tau}}{e^{S(v,v^{+})/\tau}+\sum_{j=1}^{M}e^{S(v,v_{j}^{-})/\tau}}\right], $$ 
...
• Their last actions are identical, denoted as  $ A_{S}^{*} $.
...
<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//2a3ae052-913b-463c-9327-910eaf38c4af/markdown_0/imgs/img_in_image_box_229_155_998_679.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A15Z%2F-1%2F%2F179fc29c4c48669cd8a1142ef9a38652c635f7f00d7bf6ea183516d8db574dc6" alt="Image" width="62%" /></div>


<div style="text-align: center;"><div style="text-align: center;">Figure 4: The overall architecture of MobiFlow</div> </div>
...
<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//2a3ae052-913b-463c-9327-910eaf38c4af/markdown_2/imgs/img_in_chart_box_219_141_468_291.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A17Z%2F-1%2F%2F945ea1a216b9554858fc5f4e7679940c4f0b2db88e91943d24d50aa391b0db31" alt="Image" width="20%" /></div>


<div style="text-align: center;"><div style="text-align: center;">(a) Overall Average Score</div> </div>


<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//2a3ae052-913b-463c-9327-910eaf38c4af/markdown_2/imgs/img_in_chart_box_488_142_737_291.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A17Z%2F-1%2F%2F495afdcd1df39c122b18cc5c18ba0b9f0c1f29e699e00b517947aa63a334c2c3" alt="Image" width="20%" /></div>


<div style="text-align: center;"><div style="text-align: center;">(b) Average Easy Task Score</div> </div>


<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//2a3ae052-913b-463c-9327-910eaf38c4af/markdown_2/imgs/img_in_chart_box_757_142_1005_290.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A17Z%2F-1%2F%2F6733f6e407ae4ce34db85b7099aa25565bee3bf9b30feb60f115574ee6fdb9e2" alt="Image" width="20%" /></div>


<div style="text-align: center;"><div style="text-align: center;">(c) Average Hard Task Score</div> </div>


<div style="text-align: center;"><div style="text-align: center;">Figure 5: Average Task Completion Scores of Different Agent Models Under Realistic Workloads (MobiFlow): The performance is shown for (a) all tasks combined; (b) easy tasks; (c) and hard tasks.</div> </div>
...
<div style="text-align: center;"><img src="https://pplines-online.bj.bcebos.com/deploy/official/paddleocr/pp-ocr-vl-15//2a3ae052-913b-463c-9327-910eaf38c4af/markdown_3/imgs/img_in_chart_box_221_150_1001_475.jpg?authorization=bce-auth-v1%2FALTAKDN8mY5KlNI7zaRpLmOqrw%2F2026-05-03T07%3A47%3A19Z%2F-1%2F%2F05d9971c03cade2fd5594d36ceae19c1bb15ca9978bf2e71e5c48726ff6c3f85" alt="Image" width="63%" /></div>


<div style="text-align: center;"><div style="text-align: center;">Figure 6: Action Replay Rate with AgentRR under Different Distributions.</div> </div>
...
[9] Wenyi Hong, Weihan Wang, Qingsong Lv, Jiazheng Xu, Wenmeng Yu, Junhui Ji, Yan Wang, Zihan Wang, Yuxuan Zhang, Juanzi Li, Bin Xu, Yuxiao Dong, Ming Ding, and Jie Tang. 2024. CogAgent: A Visual Language Model for GUI Agents. arXiv:2312.08914 [cs.CV] https://arxiv.org/abs/2312.08914

[10] Raghav Kapoor, Yash Parag Butala, Melissa Russak, Jing Yu Koh, Kiran Kamble, Waseem Alshikh, and Ruslan Salakhutdinov. 2024. OmniACT: A Dataset and Benchmark for Enabling Multimodal Generalist Autonomous Agents for Desktop and Web. arXiv:2402.17553 [cs.AI] https://arxiv.org/abs/2402.17553
...
[33] Zhuosheng Zhang and Aston Zhang. 2024. You Only Look at Screens: Multimodal Chain-of-Action Agents. arXiv:2309.11436 [cs.CL] https://arxiv.org/abs/2309.11436

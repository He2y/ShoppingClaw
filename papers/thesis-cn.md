...
综合上述四个主题，现有研究留下四个相互关联的缺口。**缺口一：能力—成本两难**——纯模型路线要么依赖昂贵的云端大模型与高频调用，要么受限于端侧小模型的长程能力短板，token 级、查询级与步级的大小模型协同均未提供任务内里程碑粒度的状态化监督。**缺口二：无状态重复探索**——多数智能体每个任务从零开始，已被反复验证的机械导航路径每次仍需消耗模型推理。**缺口三：无生命周期的图谱**——结构化记忆普遍离线一次性构建、边无可靠性度量、无后条件验证，无法应对真实应用的持续界面演化。**缺口四：二元的图谱—模型控制**——图谱要么只做提示不省调用、要么直接接管无回退，缺少逐边区分控制强度的机制。其中缺口二至缺口四对应本文核心创新点(1)（AMSG 空间图谱），缺口一对应创新点(2)（里程碑监督）；而创新点(3)（机械护栏）所针对的小模型确定性判断不可靠问题，来自本文 3.2 节真机实测揭示的失败模式而非上述文献缺口。
...
第 9 章总结全文工作，重申创新点及其验证结论，讨论"为何不训练基座模型"的设计权衡，分析系统局限并展望后续研究方向。
...
本章回答"系统要做什么、整体长什么样"两个问题：从移动端购物场景出发给出功能性与非功能性需求，剖析小模型驱动 GUI 智能体面临的两类不确定性及其失败模式，由此引出以检索增强弥补知识短板的设计思路与四类检索通道，最后给出"三区域 + 监督层"的总体架构、闭环数据流与九阶段执行循环。本章建立的"不确定性 → 失败模式 → 设计应对"映射与总体架构框架，是第 4、5、6 章详细设计的索引与追溯主线。
...
这四类失败模式有一个共同特征：**它们不能靠提示工程或"让模型再想一遍"可靠消除**——焦点丢失与幻觉正是模型能力上限的体现，要求模型自我纠正在逻辑上是循环论证。这一观察直接导出 3.3 节以检索增强弥补知识短板的设计思路。
...
3.2 节的分析指向一个统一的设计出发点：小模型的失败大多源于"该有的知识它没有、也记不住"——不知道当前页该怎么操作才能到达下一个目标（导航知识缺失），不知道用户惯常的品牌与价位（偏好知识缺失），记不住几步前已经完成了什么（长程事实缺失）。本文据此不走"扩大模型规模"或"每步求助云端大模型"两条路，而是把这些知识沉淀为外部知识库，在每个决策点检索出来注入小模型的上下文——即以检索增强（Retrieval-Augmented Generation, RAG）弥补小模型的能力短板。

按"检索什么知识、由什么引擎检索"，系统组织为四类异构检索通道，如表 3-4 所示。

表 3-4 多源检索增强体系的四类检索通道

| 检索通道 | 检索引擎 | 检索内容 | 检索方式 | 详见 |
|---|---|---|---|---|
| 图结构检索（导航知识） | Neo4j / AMSG | 跨会话页面状态图谱：从哪到哪、怎么走 | 复合索引定位 + Dijkstra 最短路 + 路径缓存 | 第 4 章 |
| 向量语义检索（偏好知识） | FAISS | 用户历史品牌、价位偏好 | 嵌入向量近似最近邻召回 | 第 5、7 章 |
| 按需上下文检索（动态事实） | 检索网关 | 候选商品、购物车计算、历史步骤 | 思考信号触发、按需注入、带冷却 | 第 7 章 |
| 记忆事实检索（抗遗忘） | 会话记忆文件 | 已验证事实、子任务进度 | 每步结构化渲染注入 | 第 5 章 |

四类通道遵循一个朴素的分工思路：每一类知识交给最适合检索它的引擎，而不强求单一模型在参数里记住一切。其中**图结构检索是本文的核心**——它不只检索导航知识，更能让检索结果直接驱动执行、跳过模型推理（第 4 章），是检索增强从"改善提示"迈向"替代推理"的关键。在这一检索层之上，还需两重保障才能在高风险购物场景中可靠工作：被检索的事实必须为真——由会话记忆文件只记录经截图核实的事实来保证（第 5 章）；检索—决策—执行的链路不能被小模型的幻觉带偏——由机械护栏以零云调用的确定性代码接管数值比较与完成判定来保证（第 6 章）。检索引擎、事实维护与链路护栏，正对应本文的三个创新点。

## 3.4 系统总体架构

### 3.4.1 三区域与监督层

系统据此组织为**三个逻辑运行区域加一个监督层**，承载上述四类检索通道，总体架构如图 3-1 所示。理解该架构的关键在于数据怎么流动、谁决定什么：每一类决策都交给最适合它、失败代价最小的主体——图谱与机械护栏处理确定性的导航与判断，小模型负责屏幕内的元素 grounding，强 VLM 只在任务开始与里程碑处低频裁量。

图 3-1 系统总体架构（监督层：MilestoneSupervisor 三职责 + SessionMemoryFile；决策区域：_execute_step_impl 主控、TaskPlan、机械护栏、SpecGuard、三档调度；图谱区域：GraphRuntimeController 定位/Dijkstra/RuntimeDAG/ActionAdvisor/EdgeLifecycle，存储 Neo4j/FAISS/JSON；执行区域：ModelClient/ModelProtocolBridge/ActionHandler/DeviceFactory；含执行后截图反馈回路）
...
四个部分通过一条闭环数据流连接，如图 3-2 所示：截图与页面分类（浮层优先/自洽性）→ 异常看门狗与验证检测 → 图谱定位与上步后条件验证 →（条件命中时）里程碑 checkpoint 修订记忆与计划 → 三档调度门控 → 设备执行 → 新截图后条件验证与成功率更新 → 反馈回感知端；模型输出 finish 时进入完成判定（监督者截图核实或机械到访门），确认则任务结束并触发"暂存 → 质量门 → Neo4j"的图谱持久化，否决则上下文重置回退重新分析。

图 3-2 闭环数据流（快速路径、模型路径、里程碑介入与完成判定四类节点以不同颜色区分；否决分支回流至感知端构成恢复闭环）
...
初始化完成后，主循环反复调用 `_execute_step_impl()` 执行单步，每步内部按固定顺序经过九个阶段，如图 3-3 所示。

图 3-3 九阶段执行循环泳道流程图（纵向泳道，左侧标注 Phase 序号，菱形为分支判断，红色块为提前返回的人工接管/终止出口）
...
本章给出了四项功能性需求与五项量化的非功能性需求（后者将在第 8 章逐项验证），将系统不可靠性的来源刻画为环境与模型两类不确定性，并结合真机实测的四类小模型失败模式建立了"不确定性 → 失败模式 → 设计应对"的全文追溯映射（表 3-3）。针对这些失败背后的知识缺失，本章提出以检索增强弥补小模型短板的设计思路，给出图结构、向量、按需与记忆事实四类检索通道（表 3-4），并据此给出"三区域 + 监督层"的总体架构、闭环数据流与九阶段执行循环。其中空间图谱、监督层与机械护栏体系分别构成本文三个创新点的主体，详细设计依次见第 4、5、6 章。

# 第4章 AMSG 空间图谱与三档调度

作为本文的核心创新，本章设计主动移动空间图谱（Active Mobile Spatial Graph, AMSG）——导航知识的自进化检索引擎。第 3 章已指出，小模型长程失败的根源之一是缺乏可复用的导航知识：无状态的智能体每个任务都从零探索相同路径，"打开首页—点击搜索框—输入关键词—进入结果页"这样的机械导航序列在每次购物任务中重复出现，却每次都消耗完整的模型推理，任务结束后这些被实际执行验证过的导航知识又随对话上下文一起被丢弃。AMSG 把这类知识沉淀为跨任务持久、可检索、能自更新的外部图谱，并让检索结果直接驱动执行——这正是本文检索增强体系中图结构检索通道的引擎，也是创新点(1)的主体内容。本章采用系统集成视角展开：4.1 节在机制层面辨析现有图谱方法的不足并提出三条对应的设计洞察，4.2 节至 4.8 节依次给出页面状态抽象、多App组织与领域模式、锚定性分类与三档调度、搜索Compound边合成、延迟后条件验证与边生命周期、RuntimeDAG运行时集成以及持久化管线与质量门。图谱的完整技术设计文档随系统一并开源（代码仓库 `phone_agent/docs/AMSG_DESIGN.md`），本章收录其中支撑创新点论证所必需的内容，并聚焦图谱如何嵌入第 3 章定义的九阶段执行循环。
...
第二项原则是护栏与强 VLM 监督的**层次互补**：监督工作在语义层——理解任务进度、看截图核实事实、修订子任务计划，这些判断需要视觉语义理解能力，无法用规则替代；护栏工作在符号层——数字、证据、状态机计数，这些判断不需要任何模型参与。两层各守其位，互不越界：监督者不做数字比较（其输入中的机械事实快照恰恰由代码预先算好），护栏不做语义推断（不试图理解"用户到底想买什么"）。这一分工延续了本文的总体设计思路——每一类判断交给最适合它的层：语义判断归监督者、符号判断归护栏。
...
|---|---|---|
| (2) 针对纯小模型在长程任务上的焦点丢失、幻觉与伪完成问题，提出强 VLM 里程碑监督机制与会话记忆文件设计，以约 1+步数/5 次的低频云调用预算替代每步监督 | 实验一：三档消融矩阵（8.3.3） | 在成功率—延迟—云调用三维上比较纯小模型、里程碑监督与每步规划三档，检验低频监督对每步监督可靠性的逼近程度 |
| (3) 针对小模型的数字判断幻觉与伪完成，设计机械反幻觉护栏体系与确定性故障恢复机制，纯代码确定性判断、零额外云调用，每条可能循环的恢复路径均有有界终止保证 | 实验二：护栏消融（8.3.4） | 分别关闭单项护栏，度量对应失败模式的发生率回升与终止保证是否被破坏 |
| (1) 针对无状态重复探索与静态图谱失效问题，设计 AMSG 自更新空间图谱与锚定性三档调度，使机械导航跳过模型推理并经边生命周期实现 UI 漂移自修复 | 实验三：图谱消融（8.3.5） | 图谱开/关与先验开/关对比，观察 Fast Path 命中率、延迟演化与自修复过程 |
...
实验一是本章核心，验证创新点 (2)。系统在实现层面预留了互斥的三档监督配置（第 5 章 5.7 节），构成天然的消融矩阵，如表 8-4 所示。
...
实验二验证创新点 (3)。在档位 B（默认配置）基础上，逐项关闭单一护栏构造消融组，每组与完整配置在同一任务子集上对比，设置如表 8-5 所示。
...
消融范围与第 6 章护栏体系（表 5-1 七项护栏）的对应关系如下：G1—G3 覆盖伪完成、价格幻觉与空响应这三类失败代价最高、且具备可观测发生率指标的护栏；其余护栏中，异常看门狗与验证检测由功能测试 FT-06 验证，搜索词净化由 FT-01 验证，finish 防洗白属于代码不变量、由代码审计与单元测试验证，均不在消融矩阵内单独设组。相应地，创新点 (3) 中"经消融验证的拦截贡献"仅限上述三类高代价失败，其余护栏的有效性结论依据功能测试与代码审计。

### 8.3.5 实验三：图谱消融

实验三验证创新点 (1)，包含三组对照与一项案例分析。第一组为图谱开/关对比：关闭图谱区域后系统退化为每步完整模型推理（保留监督与护栏），与完整配置对比平均步数、端到端延迟、Fast Path 命中率与 DAG 命中率（含节省的分类器调用次数）。第二组为冷启动对照：设置 `AMSG_DOMAIN_PRIORS=0` 关闭同域结构先验，在京东（以淘宝图谱为既有同域知识）上比较冷启动阶段的探索效率，验证"同域结构先验仅以文本提示注入、不携带坐标"设计的冷启动收益。第三组为时间演化观测：在同一应用上连续执行任务，记录冷启动期、早期与稳定期的 Fast Path 命中步数与端到端延迟演化曲线。随系统开源的设计文档（代码仓库 `phone_agent/docs/AMSG_DESIGN.md` §6.2）给出的延迟演化表（冷启动约百秒级、稳定期降至约三至四成，表头自标"估算"）为预期趋势假设，本实验以实测曲线检验该趋势，实测值【实验数据待补充】。
...
本文围绕"如何让可量化端侧部署的小型 GUI 模型在长程移动购物任务上达到接近大模型的可靠性"这一核心命题，提出以检索增强弥补小模型知识短板的总体思路，设计并实现了基于检索增强的移动端购物智能体 Shopping-Agent。系统把完成长程购物任务所需的导航、偏好与长程事实知识沉淀为可检索、自更新、有质量门的外部知识库，在每个决策点检索出来增强小模型，并以"三区域 + 监督层"的架构让每一类决策交给最适合它的主体：空间图谱检索并直接执行机械导航，纯代码的机械护栏接管确定性判断，小模型保留对当前屏幕的 grounding 权威，强 VLM 仅在任务开始与里程碑时刻低频介入。系统通过模型协议桥支持 AutoGLM、UI-TARS、Qwen-VL、MAI-UI、GUI-Owl 五个 GUI 模型家族，通过设备抽象层覆盖 Android（ADB）、HarmonyOS（HDC）与 iOS（XCTest）三个设备平台，并已在淘宝与京东两个真实购物 App 上完成端到端验证。本文的主要创新点总结如下。

（1）提出了 AMSG 自更新空间图谱与锚定性三档调度。针对无状态智能体在机械导航上的重复探索问题，将历史轨迹沉淀为以 (app, page_type) 为节点的 Neo4j 页面状态图谱，以"假设—候选—提升—降级"的边生命周期统计保证转移可靠性，并按边的锚定性进行快速路径/图谱副驾/模型路径三档调度，配合 RuntimeDAG 缓存与搜索 Compound 边降低每步开销，且以延迟后条件验证保证快速路径失配时的安全回退。第 8 章设计了图谱消融实验，以检验图谱调度能否在机械导航段减少模型调用并缩短任务时长，量化结论待实验完成后回填【实验数据待补充】。

（2）提出了强 VLM 里程碑监督机制与会话记忆文件设计。针对小模型在长程任务中的焦点丢失与计划遗忘问题，强 VLM 通过 initialize/checkpoint/final_confirm 三职责在任务开始与里程碑处低频介入（设计预算约每任务 1+步数/5 次调用），以原始任务永不改写的会话记忆文件作为监督层与执行器之间的解耦接口和抗遗忘持久锚，配合四触发器去抖、原子修订与"失败即维持现状"的降级语义保证机制鲁棒性。第 8 章设计了三档消融实验，以检验里程碑监督能否在云调用次数与延迟远低于每步规划上界的同时，相对纯小模型基线显著提升长程任务可靠性，量化结论待实验完成后回填【实验数据待补充】。

（3）设计了机械反幻觉护栏体系与确定性故障恢复机制。针对小模型的数字判断幻觉、伪完成与空响应等真机实测失败模式，将价格预算对比、购买提交点拦截（SpecGuard）、完成证据核实等确定性判断从模型推理中剥离，交由零额外云调用的规则代码执行，并以单一完成权威避免双判定路径互相误拦；每一类故障模式均映射到确定性响应，每条可能循环的恢复路径均有有界终止保证，杜绝"既不结束也不推进"的死循环。第 8 章设计了护栏消融实验，以度量护栏体系对价格越界购买与虚报完成等高代价错误的拦截贡献，量化结论待实验完成后回填【实验数据待补充】。

## 9.2 设计权衡讨论：为何不训练基座模型

近年来，相当一部分移动 GUI 智能体工作通过强化学习或监督微调直接优化基座模型，以提升其在真实任务上的表现。本系统采取了相反的取向——不改动基座 GUI 模型，而以外层的图谱、护栏与低频监督结构提升可靠性。本节论证这一取向在移动购物问题域上的合理性，并界定其适用边界，以回应"为何不引入模型训练"这一自然质疑。

不训练基座的判断基于三点。其一，**高代价的失败属于确定性判断，不应交给训练**。本文实测的失败模式中代价最高的两类——价格越界误判与伪完成——本质上是符号计算与证据核实，机械护栏以零额外成本保证其正确（第 6 章）；训练即便能让模型逼近正确，也无法超越"保证正确"。更进一步，若以"后条件匹配"作为奖励信号训练执行器，模型将有动机迎合而非正确触发页面分类器，这与本文页面分类器独立性红线（4.6.3 节）直接冲突，反而会侵蚀边生命周期统计的真实性。

其二，**高层失败宜以机制绕开，而非以训练硬扛**。长程任务的焦点丢失源于小模型有限的长上下文管理能力，属模型容量层面的结构性短板；本文以外置的会话记忆文件（持久化原始任务 + 每步注入已验证事实）绕开这一短板，比训练一个端侧小模型去硬扛长程记忆更可靠、也更经济（第 5 章）。当一个失败可由确定性机制以更低成本消解时，将其转化为训练目标并非合理的资源分配。

其三，也是问题域层面最根本的一点：**快速变化的知识宜以可自更新的非参数化外部记忆承载，而非固化进模型权重**。移动购物应用的内容与布局高频更新，任何固化进权重的导航知识都会随版本迭代失效，而权重更新需要重新训练，成本高、周期长；相反，本文的自进化空间图谱以在线持久化在每次成功任务后低成本增量更新，失效的边经后条件验证自动降级、重探索、重提升（第 4 章）。在快速变化的知识上，可廉价自更新的非参数化外部记忆，结构性地优于需要重训才能更新的参数化权重——这正是本系统将导航知识置于图谱而非模型的根本原因。

需要诚实界定这一论证的边界。上述三点在**增量漂移**场景下成立：按钮移位、流程小改时，自进化图谱通过"降级—重探索—重提升"自愈，这是其最强的场景，也是"应用高频更新"这一前提对本设计有利的一面。但对**全新应用或整体改版的冷启动**，图谱为空，系统退化为纯模型推理，这是该范式已知的、也是唯一值得继续投入的软肋（如本文京东子图在初期缺少经验证的可执行边）。值得强调的是，冷启动的解法是**加速数据飞轮**——引导式探索、同域结构先验注入与人工辅助采集（第 4 章）——而非模型训练：因为冷启动恰恰缺少训练所需的成功轨迹，且训练得到的版本知识在应用再次更新后会同样过时。

最后补充一点对称的观察：本文的图谱构建管线与在线持久化机制，配合后条件验证与机械护栏所构成的自动可验证奖励，客观上为离线自我改进（如在已验证成功的轨迹上做拒绝采样微调）提供了近乎零人工标注成本的数据基础。但如上分析，对移动购物这一高频更新、且高代价失败已由确定性护栏覆盖的问题域而言，训练基座的边际收益有限；故本文未将模型训练纳入系统设计，而将其作为一个对照性的开放问题，留待后续在更大规模数据上检验。
